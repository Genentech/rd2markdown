...
The `rd2markdown` itself is a dependency-less, base R centric package that converts `.Rd` documentation objects into markdown files. We think the use case that prompted us to create this package is only one of the many various applications of converting `.Rd` to `.md` functionality, as we have underlined in the first paragraph. There have been already a few attempts to resolve the problem of documentation parsing, but none of the solutions were satisfactory enough. We aim to provide a solution that, with a help of the R community, will satisfy requirements set by various use cases and groups.
...
The `rd2markdown` is the core of the `rd2markdown` package. It is here, that the actual conversion from `.Rd` into `.md` happens. It uses the innovative idea of treating particular Rd tags as classes and dispatching to particular methods accordingly. Thanks to that, most of the common tags have dedicated treatment and can be formatted in various ways. On top of that, such a design ensures trouble-free extensibility of the package, should new tags need to be covered. 
...
`rd2markdown` function can also work with a path to the file taking advantage of the hidden parameter `file`

```{r}
md <- rd2markdown::rd2markdown(file = rd_file_example)
cat(md)
```
...
```{r}
md <- rd2markdown::rd2markdown(topic = "rnorm", package = "stats", fragments = c("description", "usage"))
cat(md)
```
...
The `get_rd` function actually extracts the .Rd file from the file or directly from the package. It can be used on its own, but also `rd2markdown` function uses it internally. The function itself is using the base function `tools::parse_Rd` and `base::help` to extract the documentation while escaping errors with graceful exceptions handling. 
...
`documentation_to_markdown` function merges both and `rd2markdown` and `get_rd` providing a vectorized way of parsing multiple documentation files at once. It is enough to just pass a vector of paths and the function will return a named list of markdown documents created with `rd2markdown` function.
...
The `rd2markdown` package is an attempt to solve the issue of converting R package documentation objects into readable markdown documents. It is still in its early development stage, so not every single aspect or nuance is covered. Therefore, we would like to encourage every community contribution by opening the issue on https://github.com/genentech/rd2markdown and describing the problem or missing functionality. We ] value any type of input as we aim to provide a tool that will be possible to use in various R applications.
